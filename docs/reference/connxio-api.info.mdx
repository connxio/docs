...
<h1 className={"openapi__heading"}>Connxio API</h1>
...
<div style={{ marginBottom: "2rem" }}>
  ...
  <SchemaTabs className={"openapi-tabs__security-schemes"}>
    <TabItem label={"API Key: Connxio-Api-Key"} value={"Connxio-Api-Key"}>
      API Key for Connxio API. Can also be set using a query parameter. If using
      the webhook functionality, the header 'Connxio-Api-Webhook' must also be
      set to 'true'
      <div>
        <table>
          <tbody>
    ...
    <TabItem label={"API Key: Connxio-Api-key"} value={"Connxio-Api-key"}>
      API Key for Connxio API as a query parameter. Only use if headers are
      unavailable to you.
      <div>
        <table>
          <tbody>
    ...
    <TabItem
      label={"API Key: Connxio-Api-Webhook"}
      value={"Connxio-Api-Webhook"}
    >
      This header must be set to 'true' to enable webhook functionality for the
      API key
      <div>
        <table>
          <tbody>
...
>
  ...
  <span>Evidi: [support@connxio.no](mailto:support@connxio.no)</span>
  <span>URL: [https://www.evidi.com](mailto:https://www.evidi.com)</span>
</div>
