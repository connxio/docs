...
To use the synchronous functionality, set up a regular [API Inbound Adapter](./adapters/inbound/api.mdx) and toggle the _Use Synchronous Communication_ option.
...
You can customize the API output with a [Code component](./transformation/code-components.md) or a [Script component](./transformation/script.md). Use the following records from the [Connxio.Transformation](https://www.nuget.org/packages/Connxio.Transformation) NuGet package to access synchronous API response data.
...
See the [Code Component](./transformation/code-components.md) documentation for information about how to change the return code.
