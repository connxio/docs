...
When using the *Use event as Content* option you are required to handle the event manually. The easiest way to accomplish this is to extract the blob name from the `Url` parameter with a Code Mapping and use it with the [Blob Get transformation](../../blob-get.md).
