...
- **Code Acknowledgement Map (Code Component)**: The _Code Component_ is selected as described in the [Code Components](../../transformation/code-components.md) section.
...
Creating an Ack code component is done in more or less the same way as [map code components](../../transformation/code-components.md). The only differences are that an Ack code componenet requires the 'IConnXioAck' interface and that is takes 'bool success' as a parameter.
