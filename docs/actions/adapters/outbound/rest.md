...
The behavior is controlled by the Sequential Delivery Correlation Type, which decides the correlation key used to route messages into a shared subscription on the outbound pipeline:
...
Messages are delivered in sequence based on the order they are received, but ordering is not guaranteed. Only a single message is processed at one time for the chosen correlation group. The next message is only picked up when the current one is processed and the _Delivery Delay_ has expired.
...
If you need to receive the InterchangeId by other means we recommend either including it in the message by using [transformations](../../code-components.md) or adding the InterchangeId as a query parameter with [Connxio Macro Language](../../../integrations/cxmal/connxio-macro-language.md).
