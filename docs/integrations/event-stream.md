...
To create an Event Stream you configure your inbound adapter as normal by choosing an adapter and adding all the required fields. When the adapter is done you **right click** the adapter box and select **Convert to event stream** like shown here:
...
An Event Stream source is not of any value until you have a consumer that processes the events generated by it. To create a consumer you open an already existing integration or create a new one. In the inbound _Trigger_ box you click on the `Add Trigger` button and select _Event Stream_ like this:
